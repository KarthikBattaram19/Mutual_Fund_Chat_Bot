...
def test_vercel_config_exists() -> None:
    ...
    assert vercel["outputDirectory"] == "ui"
    assert vercel["buildCommand"] == "npm run build"
    assert package["scripts"]["build"] == "node scripts/generate_ui_config.js"


def test_generate_ui_config_writes_api_base_url() -> None:
    config_path = Path("ui/config.js")
    before = config_path.read_text(encoding="utf-8")
    try:
        env = {**os.environ, "API_BASE_URL": "https://api.example.railway.app"}
        subprocess.run(["node", "scripts/generate_ui_config.js"], check=True, env=env)
...
def test_ui_loads_runtime_config_before_app() -> None:
    soup = BeautifulSoup(Path("ui/index.html").read_text(encoding="utf-8"), "html.parser")
